...
def prepare_help(root: Path, existing: bool = True, legacy: bool = True) -> dict[str, Path]:
    ...
    paths["source"].write_text(
        CSV_HEADER
        + "demo,demo-setup,Setup Demo,SD,Setup,configure,,anytime,,,false,out,config\n",
        encoding="utf-8",
    )
    if existing:
        ...
        paths["target"].write_text(
            CSV_HEADER
            + "demo,stale,Stale,S,Stale,run,,anytime,,,false,out,old\n"
...
def test_valid_existing_merges_preserve_semantics_modes_and_cleanup():
    ...
    for script in CONFIG_SCRIPTS:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert "stale" not in config["demo"]
    ...
    for script in HELP_SCRIPTS:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert [row[0] for row in rows[1:]] == ["Other Module", "demo"]
...
def test_aliases_case_collisions_and_nested_outputs_are_rejected():
    ...
    for script in HELP_SCRIPTS:
        for variant in ("hard-link", "case", "nested", "target-cleanup", "source-cleanup"):
            with tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                ...
                if variant == "hard-link":
                    ...
                    paths["target"] = paths["source"] / "module-help.csv"
...
def test_cleanup_help_source_requires_one_module_and_accepts_installed_bmb_asset():
    ...
    for script in HELP_SCRIPTS:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            paths = prepare_help(root, legacy=False)
            source_text = BMB_HELP_SOURCE.read_text(encoding="utf-8")
            rewritten = [source_text.splitlines(keepends=True)[0]]
            for line in source_text.splitlines(keepends=True)[1:]:
                if "," in line:
                    rewritten.append("bmb," + line.split(",", 1)[1])
                else:
                    rewritten.append(line)
            paths["source"] = root / "bmb-module-help.csv"
            paths["source"].write_text("".join(rewritten), encoding="utf-8")
            paths["legacy"] = root / "legacy"
            cleanup_files = []
...
def test_unresolved_project_root_argv_fails_before_mutation():
    """Literal {project-root} path arguments are rejected before any publication."""
    for script in CONFIG_SCRIPTS:
        with tempfile.TemporaryDirectory() as temporary:
            paths = prepare_config(Path(temporary))
            pair_before = [snapshot(paths[name]) for name in ("config", "user")]
            legacy_before = [snapshot(path) for path in legacy_files(paths, "config.yaml")]
            arguments = config_arguments(paths)
            arguments[1] = str(paths["config"].parent / "{project-root}" / "config.yaml")
            code, _, _ = run_cli(script, arguments)
            assert code == 1, script
            for name, expected in zip(("config", "user"), pair_before):
                assert_snapshot(paths[name], expected)
            for path, expected in zip(legacy_files(paths, "config.yaml"), legacy_before):
    ...
    for script in HELP_SCRIPTS:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            target_before = snapshot(paths["target"])
            legacy_before = [snapshot(path) for path in legacy_files(paths, "module-help.csv")]
            arguments = help_arguments(paths)
            arguments[1] = str(paths["target"].parent / "{project-root}" / "module-help.csv")
            code, _, _ = run_cli(script, arguments)
            assert code == 1, script
            assert_snapshot(paths["target"], target_before)
            for path, expected in zip(legacy_files(paths, "module-help.csv"), legacy_before):
...
def test_result_templates_transform_answers_and_reject_non_strings():
    """String result templates are applied; a non-string result is a validation error."""
    for script in CONFIG_SCRIPTS:
        with tempfile.TemporaryDirectory() as temporary:
            paths = prepare_config(Path(temporary), legacy=False)
            paths["module"].write_text(
                "code: demo\n"
                "name: Demo Module\n"
                "description: Demo description\n"
            ...
            assert code == 0, (script, stdout, stderr)
            config = yaml.safe_load(paths["config"].read_text(encoding="utf-8"))
            assert config["demo"]["choice"] == "{project-root}/answer-value"
            assert_no_artifacts(Path(temporary))
        with tempfile.TemporaryDirectory() as temporary:
            paths = prepare_config(Path(temporary))
            paths["module"].write_text(
...
def test_ci_runs_canonical_suite_as_blocking_no_bytecode_gate():
    """The normal workflow policy job blocks on the canonical merge-atomicity suite."""
    workflow = CI_WORKFLOW.read_text(encoding="utf-8").replace("\r\n", "\n")
    expected_step = """      - name: Validate BMAD merge atomicity
        env:
    ...
    assert "test-merge-atomicity.py" in workflow_gates
    assert workflow_gates.count("test-merge-atomicity.py") == 1
...
if __name__ == "__main__":
    tests = [
        ...
        test_unresolved_project_root_argv_fails_before_mutation,
        test_result_templates_transform_answers_and_reject_non_strings,
