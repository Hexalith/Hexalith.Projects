...
def read_csv_rows(path: str) -> tuple[list[str], list[list[str]]]:
    ...
    try:
        ...
        raise ValidationError(f"Could not parse CSV from {path}: {error}") from error
    if not rows:
        return [], []
...
def run(args) -> dict:
    ...
    if is_nested_path(target, source):
        raise ValidationError("help CSV source and target paths must not be nested")
    ...
    if args.legacy_dir:
        ...
        source_module = next(iter(source_codes))
        if source_module != args.module_code:
            raise ValidationError(
                f"source module-column value {source_module!r} must equal --module-code {args.module_code!r}"
            )
