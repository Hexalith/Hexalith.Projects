...
/// never satisfy a query for another tenant). Runtime hosts replace this test/pre-runtime model with
/// the Dapr-backed projection store; the projection itself is the reusable, rebuildable read model.
/// </summary>
public sealed class InMemoryProjectDetailReadModel : IProjectDetailReadModel
{
    ...
    /// <summary>
    /// Folds a single project event into the projection (the same dispatch shape the Workers projection
    /// subscriber feeds into the durable store). Pure projection update; tenant-guarded by the envelope/event
