...
The script validates the complete deduplicated target list before inspecting target contents: every `--module-code` and `--also-remove` value must name one non-symlink direct child of the resolved `_bmad/` root, and distinct requested names must not resolve to the same target. It then aborts the complete cleanup if any target still contains `config.yaml` or `module-help.csv`. For every legacy skill, the matching `.claude/skills/` directory must resolve outside all cleanup targets, differ from the legacy directory, and contain a regular `SKILL.md`; a same-named directory alone is not proof of a replacement. `--skills-dir` is required whenever any cleanup target still contains legacy skills — omitting it fails every skill in that target as unproven. Directories without live migration files or skills (like `_config/`) are removed directly after those batch checks pass. If the script exits non-zero, surface the JSON error and stop. Missing directories (already cleaned by a prior run) and non-directory targets are not errors — the script is idempotent.
