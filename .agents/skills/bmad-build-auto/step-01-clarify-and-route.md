...
- `done` → **EARLY EXIT** to `[[bmad-snapshot:step-04-review.md]]` for a fresh review pass without mutating the spec. (A `done` spec is a completed run, so step 04 starts a new ownership session and performs any reset through its gated control-mutation protocol.)
...
- **If exactly one matches**, set `spec_file` to that path.
  ...
  - Any other recognized `status`: **EARLY EXIT** using the same mutation-free routing as above. One difference: a `blocked` story HALTs with blocking condition `story already blocked`, not `blocked spec supplied` — the caller did not supply this file; build-auto found it by id.
