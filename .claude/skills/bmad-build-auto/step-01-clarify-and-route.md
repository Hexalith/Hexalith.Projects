...
- `done` → set `followup_pass` to `true`, then **EARLY EXIT** to `[[bmad-snapshot:step-04-review.md]]` for a fresh review pass. Do not reset `review_loop_iteration` here; step-04 resets it only after the fresh follow-up ownership gate. (A `done` spec is a completed run, so this starts a follow-up review, not a resumption.)
...
- **If exactly one matches**, set `spec_file` to that path.
  ...
  - Any other recognized `status`: **EARLY EXIT** using the same routing as above, including `followup_pass` for `done` and without resetting `review_loop_iteration`. One difference: a `blocked` story HALTs with blocking condition `story already blocked`, not `blocked spec supplied` — the caller did not supply this file; build-auto found it by id.
