...
- `expected_workspace` is the current exact ownership checkpoint. A complete checkpoint contains the full canonical `HEAD`; the exact index bytes and the actually referenced split-index bytes; NUL-safe porcelain-v2 records and raw path classes; binary, full-index tracked/index hunks; stable no-follow identities for every tracked and untracked path (type, mode, and content or symlink-target hash); recursively observed initialized gitlink/submodule HEAD, nested index bytes, nested gitlinks, and nested worktree path identities (type, mode, and content); and separately named spec, result, patch, triage, ledger, and other control-path identities even when ignored. A gitlink path that is a symlink or non-directory is uninitialized and must not be followed. A first pass captures this checkpoint only from a clean aligned HEAD/index/worktree. A bounded stable double capture must produce identical observations. Missing, unreadable, raced, or unsupported path types make the checkpoint invalid.
- `owned_delta` is the allowlisted, persisted-and-hashed binary/full-index patch plus path/type/preimage evidence derived from the stable pre-handoff and post-handoff checkpoints. It includes only implementation changes attributed to this handoff, including mode-only owned changes. Persist it as the `{spec_file}.owned-delta` sidecar: control-owned committed evidence that stores modes, rename pairs, path/type/preimage bytes, and the patch digest, which must be verified on load and rewritten after every authorized repair. The same-run newly planned spec is an exact control-owned creation or mutation, not a reversible implementation hunk, but must remain in finalization inputs with the sidecar.
