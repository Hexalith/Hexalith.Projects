...
- **Overall readiness:** NOT_READY (2026-07-16 independent rerun; supersedes 2026-07-15)
- **Freeze:** no corrective implementation, no 6.x/7.x/8.x story-file creation, no sprint-status reconciliation until an independent readiness rerun returns exactly READY
...
**Status legend:** `pending` = owned + fully specified, not yet executed (awaiting READY) · `blocked-external` = additionally gated on a G-1…G-6 external capability · `not-verified` = environment/tool unavailable (never a pass) · `failed` = executed and currently failing · `passed` = real, honest passing evidence (none yet). None of `pending`/`blocked-external`/`not-verified`/`failed` counts as a pass.
...
| `fr-2` | FR-2 | Open Project — authorized metadata/lifecycle/setup/reference summary via supported read models | 6.1 | — | AD-3, AD-14, AD-19, AD-32 | 🟡 pending | — |
...
| `fr-5` | FR-5 | List Projects — Tenant-scoped, authorization-filtered, cursor-paged | 6.1 | — | AD-14, AD-19, AD-20, AD-27 | 🟡 pending | — |
...
| Key | IDs | Description | Primary | Supporting | ADs | Status | Blocker |
|---|---|---|---|---|---|---|---|
| `nfr-1` | NFR-1 | Security & privacy — Tenant/actor/action/target/version scoping; fail-closed on stale auth; metadata-only logs/telemetry/errors | 8.8 | 6.1, 6.5, 7.1 | AD-11, AD-13, AD-20 | 🔒 blocked-external | G-5 identity; authenticated live topology |
...
| `nfr-5` | NFR-5 | Performance & scale — 10k Projects/Tenant, 5k refs/Project, 100k audit; reads p95 <500ms (<1s max); admission p95 <500ms warm | 8.9 | 6.7 | AD-14, AD-15, AD-27 | 🟡 pending | — |
...
| `nfr-10` | NFR-10 | Compatibility — additive/serialization-tolerant contracts; name-only creation preserved; retirement gated; no event-history rewrite | 6.7 | 7.15, 8.7 | AD-6, AD-16, AD-17, AD-22 | 🟡 pending | — |
