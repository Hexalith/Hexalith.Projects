...
- **Validation gate (target):** `dotnet tool run hexalith-evidence validate _bmad-output/planning-artifacts/implementation-readiness-traceability-matrix.yaml` — tool status **not-available**; Builds revision `b529b66` is an unaccepted source candidate, not an accepted published consumer capability
