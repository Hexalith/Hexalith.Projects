...
- .NET SDK: `10.0.302` via `global.json` with `rollForward: latestPatch`; primary projects target `net10.0`.
...
- Before sprint planning or reconciliation changes `sprint-status.yaml`, run `python3 tools/planning/validate_production_authority.py --validate-index`. Generate the candidate index separately, run `python3 tools/planning/validate_production_authority.py --validate-index --sprint-status <candidate>` against that exact candidate, and only then replace the active index atomically; re-run the active-index check afterward. P1R can close only when `_bmad-output/implementation-artifacts/6-1-p1r-acceptance.json` exists and passes the strict selected/rollback coordinate, UTC timestamp, and four-role decision schema. Halt on every nonzero result. Do not bypass, weaken, or reinterpret this Projects-owned scheduling guard.
