...
remote_consumer=/tmp/hexalith-p1r-3106-attempt9.xF8z3V/remote-consumer
remote_cache=/tmp/hexalith-p1r-3106-attempt9.xF8z3V/caches/remote-packages
...
while IFS= read -r package_id; do
    ...
    printf '%s\t3.106.0\t%s\t%s\t%s\t%s\t%s\n' \
        "$package_id" \
        "${package_directory#"$remote_cache"/}" \
        "$(sha256sum "$nupkg" | cut -d' ' -f1)" \
        "$(sha256sum "$nuspec" | cut -d' ' -f1)" \
        "$(sha256sum "$metadata" | cut -d' ' -f1)" \
        "$(sha256sum "$sha512_file" | cut -d' ' -f1)" >> "$inventory"
...
jq -e '.project.restore.packagesPath == "/tmp/hexalith-p1r-3106-attempt9.xF8z3V/caches/remote-packages"' "$retained/obj/project.assets.json" >/dev/null
...
for package_id in $(jq -r '.packages[].id | select(. != "Hexalith.EventStore.Admin.Cli")' "$manifest"); do
    jq -e --arg key "$package_id/3.106.0" '.libraries[$key] != null' "$retained/obj/project.assets.json" >/dev/null
done

(
    cd "$bundle"
    sha256sum remote-consumer/RemoteConsumer.csproj \
        remote-consumer/NuGet.config \
        remote-consumer/obj/RemoteConsumer.csproj.nuget.dgspec.json \
        remote-consumer/obj/RemoteConsumer.csproj.nuget.g.props \
        remote-consumer/obj/RemoteConsumer.csproj.nuget.g.targets \
        remote-consumer/obj/project.assets.json \
        remote-consumer/obj/project.nuget.cache \
        remote-package-inventory.tsv > remote-consumer-files.sha256
