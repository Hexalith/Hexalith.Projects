...
| Dirty follow-up entry | A `done` follow-up starts with any pre-existing index/worktree change, including orchestrator-owned ledger or control-file bookkeeping | Do not adopt, discard, stage, or commit the change; require its owner to resolve it, then restart from a clean current HEAD | Halt blocked without repository mutation or a fresh checkpoint |
| Orchestrator re-drive handoff | Re-arm changed spec status/result content or other control bookkeeping before dispatch | The orchestrator materializes the complete corrected control state in a clean commit/ref, then dispatches with aligned HEAD/index/worktree | Halt blocked without adopting or normalizing the rearm delta; the orchestrator owner must repair the handoff |
...
- Given a normal implementation, interrupted resume, committed `done` follow-up review, and orchestrator re-drive, when the workflow enters step 03 or 04, then it respectively captures once, fails closed without live ownership state, establishes a fresh expected HEAD without replacing the historical review base, or starts only from an owner-materialized clean control-state commit/ref; if the follow-up or re-drive entry is dirty, it halts without adopting or mutating the pre-existing changes and captures nothing until their owner resolves them.
...
- Confirmed the same strict boundary for orchestrator re-drives: bmad-loop must materialize rearm bookkeeping in a clean control-state commit/ref before dispatch, and Build Auto must not adopt or normalize a staged rearm delta.
