...
If a layer's instruction requires subagents and none are available, generate one review prompt file per such layer in `/home/administrator/projects/hexalith/projects/_bmad-output/implementation-artifacts` and HALT. Ask the human to run each in a separate session and paste back the findings.
...
- **defer** — pre-existing issue not caused by this change. Append one new entry to `/home/administrator/projects/hexalith/projects/_bmad-output/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates.
  ```markdown
